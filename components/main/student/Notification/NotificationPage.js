...
const NotificationPage = (props) => {
      ...
      await firebase
        ...
        .then((snapshot) => {
          let list = snapshot.docs.map((doc) => {
            ...
            return { id, title, pageId, description, creation, dataType };
          });
  ...
  const navigationFunction = (page, id, nId) => {
    if (page === 'Discussion') {
      const fid = discussionId.some((el) => el.id === id);
      if (fid) {
        props.navigation.navigate(page, { did: id, notificationId: nId });
      } else {
      ...
      if (fid) {
        props.navigation.navigate(page, { cid: id, notificationId: nId });
      } else {
        props.navigation.navigate('FallBackPage', { notificationId: nId });
      }
    }
    ...
    <NotificationList
      ...
      onSelect={() => {
        if (data.item.dataType === 'id') {
          ...
          navigationFunction('Discussion', data.item.pageId, data.item.id);
...
const styles = StyleSheet.create({
  ...
  backRightBtnRight: {
    ...
    right: Dimensions.get('window').width < 400 ? 0 : -50,
  },
});
